...
unsigned char mymac[6] = {0xf2, 0x3c, 0x94, 0x90, 0x4f, 0x4b}; // {0xf2,0x3c,0x91,0xdb,0xc2,0x98};
unsigned char mask[4] = {255, 255, 255, 0};
unsigned char gateway[4] = {212, 71, 252, 1}; //{ 88,80,187,1 };
...
int resolve_mac(unsigned int destip, unsigned char *destmac)
{
        ...
        for (i = 0; i < 6; i++)
                arp->dstmac[i] = 0;
        for (i = 0; i < 4; i++)
                arp->dstip[i] = ((unsigned char *)&destip)[i];
...
int mybind(int s, struct sockaddr *addr, int addrlen)
{
        if ((addr->sa_family == AF_INET))
        {
                ...
                if (s >= 3 && s < MAX_FD)
                {
                        ...
                        if (a->sin_port && port_in_use(a->sin_port))
                        {
                                myerrno = EADDRINUSE;
                                return -1;
                        }
                                ...
                                if (ip->proto == TCP_PROTO)
                                {
                                        ...
                                        if (i < MAX_FD)
                                        {
                                                ...
                                                if (tcb->txfirst != NULL)
                                                {
                                                        ...
                                                        if ((htonl(tcp->ack) - shifter >= 0) && (htonl(tcp->ack) - shifter - (tcb->stream_end) ? 1 : 0 <= htonl(tcb->txlast->segment->seq) + tcb->txlast->payloadlen - shifter))
                                                        { // -1 is to compensate the FIN
                                                                while ((tcb->txfirst != NULL) && ((htonl(tcp->ack) - shifter) >= (htonl(tcb->txfirst->segment->seq) - shifter + tcb->txfirst->payloadlen)))
                                                                { // Ack>=Seq+payloadlen
        ...
        if (addr->sa_family == AF_INET)
        {
                ...
                *len = sizeof(struct sockaddr_in);
                ...
                do
                {
                        ...
                        {
                                if (fdinfo[s].tcblist[i].st == ESTABLISHED)
                                { // Not Fifo Queue
                                        for (j = 3; j < MAX_FD && fdinfo[j].st != FREE; j++)
                                                ; // Searching for free d
                                        if (j == MAX_FD)
                                        {
                                                myerrno = ENFILE;
                                                return -1;
