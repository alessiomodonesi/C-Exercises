...
int main()
{
    ...
    int yes = 1;
    int len;
    char *resp_502 = "<h1>HTTP/1.1 502 (Bad gateway)\nUnable to connect</h1>\r\n\r\n";
    char *resp_504 = "<h1>HTTP/1.1 504 (Gateway Timeout)\nUnable to connect</h1>\r\n\r\n";
    ...
    while (1)
    {
        ...
        // Lettura header e salvataggio in hbuf
        ...
        if (!strcmp(method, "GET"))
        {
            ...
            // aggiungo header Forwarded
            // trovo indirizzo ip dall'hostent
            char *ip;
            sprintf(ip, "%d.%d.%d.%d,", he->h_addr[0], he->h_addr[1], he->h_addr[2], he->h_addr[3]);
            sprintf(request, "GET /%s HTTP/1.1\r\nHost:%s\r\nConnection:close\r\rForwarded: for=%s\r\n\r\n", filename, hostname, ip);
            printf("%s\n", request);
